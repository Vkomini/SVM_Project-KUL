function ts()
clear;load('santafe.mat');
close all;
for i=3:10
    [error(i),cost(i),~,~] = lssvm_timeseries(Z,Ztest,10*i);
end


figure,plot(3:10,error(3:end));xlabel('order');ylabel('MSE_{test}');title('Effect of order on MSE');
end
